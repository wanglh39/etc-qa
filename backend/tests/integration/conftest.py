...
def wait_for_mysql(host="localhost", port=3306, user="root", password="123456", database="etc_qa_test", timeout=30):
    start = time.time()
...
def _cleanup_test_data():
    try:
        conn = pymysql.connect(host="localhost", port=3306, user="root", password="123456", database="etc_qa_test")
        cursor = conn.cursor()
