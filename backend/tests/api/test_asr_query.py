...
class TestASRQueryResponse:
    ...
    def test_full_fields(self):
        resp = ASRQueryResponse(
            ...
            asr_model="FunASR",
            asr_language="zh",
            asr_segments=[{"start": 0.0, "end": 1.2, "speaker": "SPEAKER_00", "text": "ETC鎬庝箞鍔炵悊"}],
            query="ETC鎬庝箞鍔炵悊",
            standardized_query="ETC鍔炵悊娴佺▼",
            confidence="high",
            candidates=[CandidateResult(qa_id=1, question="ETC濡備綍鍔炵悊", answer="鍘婚摱琛?, score=0.9)],
            total_candidates=1,
        )
        assert resp.asr_text == "ETC鎬庝箞鍔炵悊"
...
class TestASRQueryEndpoint:
    ...
    def test_asr_query_success(self, mock_rag_service, mock_get_asr):
        ...
        mock_asr.transcribe.return_value = ASRResponse(
            ...
            model="FunASR",
            language="zh",
            segments=[SpeakerSegment(start=0.0, end=1.2, speaker="SPEAKER_00", text="ETC鎬庝箞鍔炵悊")],
        )
        ...
        assert asr_result.text == "ETC鎬庝箞鍔炵悊"
    ...
    def test_asr_query_with_category(self, mock_rag_service, mock_get_asr):
        ...
        asr_result = mock_asr.transcribe("/tmp/test.wav")
        rag_result = mock_rag_service.query(asr_result.text, "ETC涓氬姟")
        mock_rag_service.query.assert_called_with("ETC鎬庝箞鍔炵悊", "ETC涓氬姟")
    ...
    def test_asr_query_rag_failure(self, mock_rag_service, mock_get_asr):
        ...
        mock_rag_service.query.side_effect = RuntimeError("RAG鏈嶅姟寮傚父")

        asr_result = mock_asr.transcribe("/tmp/test.wav")
        try:
            mock_rag_service.query(asr_result.text)
        except RuntimeError as e:
            assert "RAG鏈嶅姟寮傚父" in str(e)
