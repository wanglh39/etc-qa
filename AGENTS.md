...
28. Pinia状态管理：stores/auth.ts响应式管理token/role/dept/模拟登录状态
29. 模拟登录修复：侧边栏实时切换+提示栏响应式+退出模拟正常工作
30. 面包屑导航：Layout顶部集成BreadCrumb组件
31. 统一错误处理：request.ts 401→清store跳登录/403→权限不足/500→服务器错误
32. 客服工作台增强：两栏布局(快捷话术+分类浏览) + 一键复制 + Enter搜索/Ctrl+Enter提交工单
