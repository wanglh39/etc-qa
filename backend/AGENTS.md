...
6. sandbox测试命令：`python -m pytest tests/ -x -q -o addopts="" --ignore=tests/integration`（`-o addopts=""`去掉coverage避免加载所有模块）
7. 本地测试命令：`C:\Users\wlh19\anaconda3\envs\etc_qa\python.exe -m pytest tests/ -x -q`
...
4. 测评运行命令格式：`C:\Users\wlh19\anaconda3\envs\etc_qa\python.exe scripts/eval/eval_xxx.py`
...
### 可观测性
- 日志：stdout + `logs/etc_qa.log`（10MB轮转，5个备份）
...
- prompt/templates/*.j2：4个模板（judge/hyde_judge/hyde/structure_ingest），git管理版本
...
1. corrections加"E T C"→"ETC"等字母空格纠错（config/asr.yaml）
2. greeting正则加"为你好""为你"（asr/ws_helpers.py）
3. 流式路径加纠错表应用（asr/websocket.py + scripts/eval/eval_asr.py）
4. RAG find_expected_id加模糊匹配 + test_questions.json加expected_qa_id手动映射
5. VAD换成numpy能量VAD + min_silence_ms=200（scripts/eval/eval_asr.py）
6. 双路召回并行（rag/recall.py）
7. ASR模型复用，不每次重新加载（asr/streaming.py）
8. 状态机（asr/ws_state.py + asr/websocket.py）
9. Milvus定期重连防too_many_pings（db/milvus_client.py）
10. 日志文件+轮转（utils/logger.py）
11. LangSmith @traceable加到QAService.query和ASRService.transcribe
12. ASR模型启动预热（asr/streaming.py warmup + app.py create_service调用），避免首通用户等模型加载10-30秒
13. tests/asr/镜像整理：拆分test_ws_helpers.py+test_ws_state.py，合并test_websocket_endpoint.py到test_websocket.py
14. 提示词文件化管理（Phase 1）：提取4个.j2模板到prompt/templates/，PromptEngine优先读文件>DB>代码fallback，git管理版本
...
- tests/asr/test_websocket.py: WebSocket端点测试(asr_stream端到端)+TestStateMachine/TestControlMessageUpdate/TestOnFinalCorrections/TestFilterPipeline
...
- tests/asr/test_service.py: test_apply_corrections_etc_spaces等
- tests/rag/test_recall.py: test_parallel_recall等
- tests/agent/test_prompt_engine.py: TestPromptEngineRender/TestPromptEngineShadowAndEdgeCases/TestPromptEngineFileTemplate（文件读取+DB降级+fallback兜底）
- tests/asr/与asr/镜像结构一一对应
