...
default_permissions = {
    ...
    "ops": json.dumps(
        ["/workbench/admin/status", "/workbench/admin/monitor", "/workbench/admin/scheduler", "/workbench/admin/alert"]
    ),
...
for role_key, perms in default_permissions.items():
    cursor.execute("UPDATE roles SET permissions = %s WHERE role_key = %s AND (permissions IS NULL)", (perms, role_key))
