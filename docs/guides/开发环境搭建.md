...
只下载部分模型：
```bash
python scripts/setup/download_models.py --models embed,rerank  # 不下ASR
python scripts/setup/download_models.py --dir /path/to/models   # 自定义目录
python scripts/setup/download_models.py --force                 # 强制重新下载
```
...
etc_qa/
├── backend/
│   ├── models/          ← 模型文件（不入Docker镜像，volume挂载）
│   │   └── hub/
│   │       └── models/
│   │           ├── BAAI/bge-large-zh-v1___5/
